...
export function getActorConfig(platform, url) {
  ...
  if (platform === "youtube") return { actorId: "utils~youtube-link", input: { url, audioQuality: "high" } };
  return null;
}
...
const YT_ANDROID_CLIENT = { clientName: "ANDROID", clientVersion: "19.29.37", androidSdkVersion: 30, hl: "pt", gl: "BR" };
...
export function extractResultFromItem(platform, item) {
  ...
  if (platform === "youtube") {
    ...
    return { mediaUrl: item.downloadUrl, ext: "mp3", duration: null, title: item.title || "", audioOnly: true };
  }
  return null;
}
