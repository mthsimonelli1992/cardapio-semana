...
export function extractResultFromItem(platform, item) {
  ...
  if (platform === "youtube") {
    ...
    // preso a IP/proxy do Google, e sem exigir plano pago pra usar via API.
    if (!item.audio_url) return null;
    return { mediaUrl: item.audio_url, ext: "mp3", duration: null, title: item.title || "", audioOnly: true };
  }
  return null;
}
