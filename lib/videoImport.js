...
export function getActorConfig(platform, url) {
  ...
  // salva o vídeo baixado no armazenamento da própria Apify (storageType "apify") em vez de
  // devolver um link direto do Google — isso evita de vez o problema de URL presa a IP/proxy
  // que travava a versão anterior.
  if (platform === "youtube")
    return {
      actorId: "epctex~youtube-video-downloader",
      input: { startUrls: [url], quality: "480", storageType: "apify" },
    };
  return null;
}
...
export function extractResultFromItem(platform, item) {
  ...
  if (platform === "youtube") {
    // Ator novo salva o arquivo no armazenamento da própria Apify — o campo vem aninhado em
    // "output.url" (hospedado por eles, não precisa mais de proxy pra baixar).
    const mediaUrl = item.output?.url || item.downloadUrl || item.url;
    if (!mediaUrl) return null;
    return { mediaUrl, ext: "mp4", duration: item.durationSeconds || null, title: item.title || "", audioOnly: false };
  }
  return null;
}
