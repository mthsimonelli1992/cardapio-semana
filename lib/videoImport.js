...
// thumbnail própria) — baixa o vídeo de novo e extrai um único frame, sem transcrição nem IA.
export async function fetchVideoCoverFrame(mediaUrl, workDir, ext) {
  const videoPath = path.join(workDir, `video.${ext}`);
  await fetchToFile(mediaUrl, videoPath);
  const coverPath = path.join(workDir, "cover.jpg");
  await execFileAsync(ffmpegPath, ["-i", videoPath, "-vf", "scale=480:-1", "-frames:v", "1", "-y", coverPath]);
