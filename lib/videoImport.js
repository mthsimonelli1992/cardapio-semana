...
// reusa o mesmo proxy residencial da Apify pra baixar o arquivo de fato.
export function getApifyProxyDispatcher(apifyToken) {
  return new ProxyAgent(`http://groups-RESIDENTIAL:${apifyToken}@proxy.apify.com:8000`);
}
...
async function fetchToFile(url, filePath, dispatcher) {
...
export async function processVideoUrl(mediaUrl, workDir, ext, groqKey, durationHint, dispatcher) {
  const videoPath = path.join(workDir, `video.${ext}`);
  await fetchToFile(mediaUrl, videoPath, dispatcher);
...
export async function processAudioOnlyUrl(mediaUrl, workDir, groqKey, dispatcher) {
  const audioPath = path.join(workDir, "audio.mp3");
  await fetchToFile(mediaUrl, audioPath, dispatcher);
