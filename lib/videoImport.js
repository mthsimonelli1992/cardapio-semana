...
export function extractResultFromItem(platform, item) {
  ...
  if (platform === "youtube") {
    // Ator novo salva o arquivo no armazenamento da própria Apify — o campo "url" é hospedado
    // por eles, não precisa mais de proxy pra baixar.
    if (!item.url) return null;
    return { mediaUrl: item.url, ext: "mp4", duration: item.durationSeconds || null, title: item.title || "", audioOnly: false };
  }
  return null;
}
